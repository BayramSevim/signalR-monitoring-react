    ...
    try {
      await axios
        ...
        .then(() => {
          setOpenModal(false);
          refreshTable();
          toastMessage('Reçete Detay Silme İşlemi Başarılı', 1);
        })
        .catch((err) => {
          toastMessage('Reçete Detay Silme İşlemi Başarısız', 2);
        });
    } catch (error) {
    ...
    <Dialog open={openModal} onClose={() => setOpenModal(false)}>
      <Grid sx={{ border: '2px solid darkgray', borderRadius: '10px' }}>
        ...
        <DialogContent>
          <Grid container>
            <Grid mt={1}>
              <Grid item>Seçilen Detayın Silinmesini Onaylıyor Musunuz?</Grid>
